function [satelliteData, satelliteNames] = read_stk_data_new_format(filename)
...
% 输出: satelliteData-卫星数据结构, satelliteNames-卫星名称列表
    ...
    for i = 1:length(satelliteIndices)
        ...
        % 解析数据 - 使用逗号分隔符
        [time, x, y, z, lat, lon, success] = parseSatelliteDataCommaSeparated(dataLines);
...
%             fieldName = sprintf('Sat%d', str2double(satNumber));
            ...
            satelliteData.(fieldName).lat = lat;
            satelliteData.(fieldName).lon = lon;
...
function [time, x, y, z, lat, lon, success] = parseSatelliteDataCommaSeparated(dataLines)
    % 解析卫星数据行 - 使用逗号分隔符
    time = []; x = []; y = []; z = []; lat = []; lon = [];
    success = false;
    ...
    for i = 1:n
        ...
        % 尝试将前4个token转换为数值
        numTokens = str2double(tokens(1:6));
        ...
        lat_temp(validCount) = numTokens(5);
        lon_temp(validCount) = numTokens(6);
    end
    ...
    if validCount > 0
        ...
        lat = lat_temp(1:validCount);
        lon = lon_temp(1:validCount);
        success = true;
    end
end