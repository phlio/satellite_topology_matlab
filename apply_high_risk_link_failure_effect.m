function updated_graph = apply_high_risk_link_failure_effect(graph_matrix, sat_lat_lon, high_risk_satellites)
% 应用高风险区域断链效应对卫星拓扑的影响
...
%   updated_graph - 应用断链效应后的更新连接矩阵
    ...
    % 打印统计信息
    fprintf('高风险区域断链统计:\n');
    fprintf('  受高风险区域影响的链路数: %d\n', affected_links_count);
    fprintf('  实际断开的链路数: %d\n', broken_links_count);
    if affected_links_count > 0
        fprintf('  断链率: %.2f%%\n', (broken_links_count / affected_links_count) * 100);
    else
        fprintf('  断链率: 0.00%% (无受影响链路)\n');
    end
end