...
%   diameter    - 各时间点的网络直径
    ...
    %% ========== 第一个图：平均路径跳数时间序列 ==========
    figure('Position', [100, 100, 800, 600], 'Name', '1000次模拟平均指标:平均路径跳数时间序列');
    plot(time_data, avg_hops, 'b-', 'LineWidth', 2);
    xlabel('时间 (秒)', 'FontSize', 11); 
    ylabel('平均路径跳数', 'FontSize', 11);
    title('平均路径跳数时间序列（1000次模拟平均指标）', 'FontSize', 12, 'FontWeight', 'bold'); 
    grid on; grid minor; % 显示主/次网格
    set(gca, 'FontSize', 10); % 设置坐标轴字体大小

    %% ========== 第二个图：网络直径时间序列 ==========
    figure('Position', [200, 200, 800, 600], 'Name', '1000次模拟平均指标:网络直径时间序列');
    plot(time_data, diameter, 'r-', 'LineWidth', 2);
    xlabel('时间 (秒)', 'FontSize', 11); 
    ylabel('网络直径', 'FontSize', 11);
    title('网络直径时间序列（1000次模拟平均指标）', 'FontSize', 12, 'FontWeight', 'bold'); 
    grid on; grid minor;
    set(gca, 'FontSize', 10);
